...
function HeroMetric({ value, label }: { value: string; label: string }) {
  ...
  return (
    <div className="border-l border-line pl-4">
      <p ref={(el) => { ref.current = el; }} className="text-gradient-accent text-2xl font-semibold tabular-nums tracking-tight md:text-3xl">
        {display}
      </p>
...
export function Hero({ onResume }: { onResume: () => void }) {
  return (
    <section id="hero" className="relative overflow-hidden pb-16 pt-32 md:pb-24 md:pt-44">
      {/* quiet ambient washes — two-tone gradient glow */}
      <div aria-hidden className="pointer-events-none absolute inset-0">
        <div className="absolute -top-[30%] left-1/2 h-[60%] w-[80%] -translate-x-1/2 rounded-full bg-gradient-to-br from-accent-500/[0.09] via-accent-400/[0.05] to-alt/[0.07] blur-[120px]" />
      ...
      <div className="relative mx-auto max-w-6xl px-5 md:px-8">
        ...
        <div className="mt-8 flex flex-wrap items-center gap-3 animate-fade-in-up" style={{ animationDelay: '180ms' }}>
          <button
            onClick={onResume}
            className="group inline-flex items-center gap-2 rounded-full bg-gradient-accent px-6 py-3 text-sm font-semibold text-accent-ink shadow-glow transition-all hover:brightness-105 active:scale-95"
          >

...
/* Line drawings for the three capability areas: hairline strokes, one
   signal element picked out. Colours come from the caller. */
export function CapabilityGlyph({
  kind,
  className = 'h-14 w-14 text-ink-3',
  accentClassName = 'text-accent',
}: {
  kind: Capability['glyph'];
  className?: string;
  accentClassName?: string;
}) {
  return (
    <svg viewBox="0 0 64 64" aria-hidden fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round" className={className}>
      {kind === 'ai' && (
        <>
          ...
          <circle cx={31} cy={24} r={3.2} className={accentClassName} stroke="currentColor" />
        </>
      )}
      {kind === 'enterprise' && (
        <>
          ...
          <circle cx={50} cy={32.5} r={3} className={accentClassName} stroke="currentColor" />
      ...
      {kind === 'strategy' && (
        <>
          ...
          <path d="M42 42V22" className={cx(accentClassName)} stroke="currentColor" />
          <path d="M18 42h28" />
        </>
      )}
    </svg>
